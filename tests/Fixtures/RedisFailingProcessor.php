...
final class RedisFailingProcessor implements ProcessorInterface
{
    private string $host;
    private int $port;
    private int $db;
    ...
    public function handle(mixed $payload): void
    {
        ...
        if (is_array($payload) && !empty($payload['fail'])) {
            $client->incr($this->prefix . 'failed:' . $this->tag);
            throw new RuntimeException('Intentional failure');
        }

        $client->incr($this->prefix . 'processed:' . $this->tag);
    }
}
